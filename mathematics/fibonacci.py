...
def fibonacci(n: int) -> int:
    ...
    """
    assert n >= 0, n
    a = [(1, np.array([[1, 1], [1, 0]], dtype=int))]  # f = [[1, 1], [1, 0]]
    i = 1
    while i < n:
    ...
    for i, x in reversed(a):
        if i < n:
            m = m @ x  # @= is not yet supported
            n -= i
    return m[0].sum()  # equivalent to (m @ np.array([1, 1], dtype=int))[0]


def fibonacci2(n: int) -> int:  # recursive version
    def step(i: int) -> np.ndarray:
        assert i >= 0, i
        if i <= 1:
        ...
        m2 = m @ m
        if i & 1:
            return m2 @ np.array([[1, 1], [1, 0]], dtype=int)
        return m2
    assert n >= 0, n
    return step(n)[1].sum()  # equivalent to (step(n) @ np.array([1, 1], dtype=int))[1]
...
def test_fibonacci():
    ...
    ]  # 64-bit int only holds up to fib(92)
